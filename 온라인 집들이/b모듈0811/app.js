...
function search(keyword, data) {
    ...
    for (let i = 0; i < indexes.length; i++) {
        ...
        let flag = false;
        //"ㅈㅈ성"
        for (let j = 0; j < keywordLen; j++) {// ㅈ , ㅈ , 성
            let keywordChar = keyword[j];// ㅈ ㅈ 성
            let dataChar = (keywordChar.match(/[ㄱ-ㅎ]/) ? dataCho : data)[index + j];
            // ㅈ => dataCho// 정,a,: => data// 0 => data
...
function highlight(keyword, data, indexes) {
    ...
    for (let i = 0; i < indexes.length; i++) {
        ...
        let remainderString = data.substring(index + length, indexes[i + 1]);
        result.push(remainderString);
    }
    return result.join('');
}
